...
from utils.predictor import predict_eta
...
st.write(
    "Predict the Estimated Time of Arrival (ETA) using the trained AI model."
)
...
if st.button(
    ...
    processed = prep.prepare_eta_data(data)

    prediction = predict_eta(processed)

    st.success(
        f" Estimated ETA: **{prediction:.2f} seconds**"
    )
